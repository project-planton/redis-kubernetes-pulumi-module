...
import (
	...
	redisnetutilsport "github.com/plantoncloud/redis-kubernetes-pulumi-blueprint/pkg/redis/network/ingress/netutils/port"
...
// ex: "kubectl port-forward -n kubernetes_namespace  service/main-redis-redis 6379:6379"
func getKubePortForwardCommand(namespaceName, kubeServiceName string) string {
	return fmt.Sprintf("kubectl port-forward -n %s service/%s %d:%d",
		namespaceName, kubeServiceName, redisnetutilsport.RedisPort, redisnetutilsport.RedisPort)
}
